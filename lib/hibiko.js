  ...
  receiveMessage = publicInterface.receiveMessage = function( callback, sourceOrigin ) {
    ...
    if ( window.postMessage && callback ) {
      ...
      if ( window.addEventListener ) {
        return window[callback ? 'addEventListener' : 'removeEventListener']('message', attachedCallback, !1);
      }

      return window[callback ? 'attachEvent' : 'detachEvent']('onmessage', attachedCallback);
